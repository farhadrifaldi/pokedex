...
export function PokemonDetailEvolution({ evolutionChain }: props) {
  const { data } = usePokemonQuery(API_BASE_URL + evolutionChain?.species.name);
  ...
  const evolutionLength = evolutionChain?.evolves_to.length;
  return (
    <div>
        ...
        <>
          ...
          <div
            className={`grid grid-cols-${
              evolutionLength && evolutionLength < 2 ? evolutionLength : 2
            } gap-3`}
          >
            {evolutionChain?.evolves_to.map((ec, index) => (
              <PokemonDetailEvolution key={index} evolutionChain={ec} />
            ))}
          </div>
        </>
      )}
